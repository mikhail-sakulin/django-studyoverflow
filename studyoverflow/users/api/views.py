...
def extend_social_login_schema(provider_name: str):
    ...
    return extend_schema_view(
        post=extend_schema(
            ...
            description=(
                "Сейчас эндпоинт недоступен (выдаст 400 ответ), поскольку для api callback_url на "
                "бекенде задан как url фронтенда (теоретического, сейчас не реализован). При этом "
                "у соцсети в настройках подключенного приложения StudyOverflow указан стандартный "
                "для данной соцсети callback_url из django-allauth (для web версии OAuth), поэтому "
                "из-за несоответствия callback_url у соцсети в настройках и на бекенде соцсеть, а "
                "затем и бекенд клиенту, вернет 400 ответ, даже если отправить на данный эндпоинт "
                "правильный code.\n\n"
                f"Принимает код авторизации ('code'), полученный пользователем от {provider_name}."
                " Через него, client_id и client_secret бекенд (также еще раз отправляется "
                "callback_url, чтобы соцсеть сверила со своим значением) получает access_token от "
                "соцсети для получения данных пользователя. Через access_token бекенд получает от "
                "соцсети данные пользователя. Если пользователь заходит впервые, создается "
                "аккаунт. При успешной аутентификации возвращаются JWT access и refresh токены и "
                "данные пользователя."
            ),
            auth=[],
