...
def validate_and_normalize_tags(tags_list: list[str]) -> list[str]:
    ...
    - тег состоит только из латинских буквы, цифр и специальных символов,
      буквы других алфавитов (не латинского) запрещены.

    Приводит имя тега к нормальному виду.
    Если длина нормализованного имени тега превышает максимальное значение, вызывает исключение.
    """
    ...
    for tag in tags_list:
        ...
        # Проверка, что имя тега состоит только из латинских букв или других специальных символов,
        # буквы других алфавитов запрещены.
        for ch in normalized_tag_name:
            # .isalpha() - проверяет, что символ - буква,
            # .isascii() - проверяет, что буква строго латинская или это спецсимвол
            if ch.isalpha() and not ch.isascii():
                raise ValidationError(
                    "Имя тега может содержать только латинские буквы. "
